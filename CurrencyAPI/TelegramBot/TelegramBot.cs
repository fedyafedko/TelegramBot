...
class TelegramBot
{
    ...
    private readonly Dictionary<long, string> userState;

    public TelegramBot(string apiURL, string telegramToken)
    {
        _apiURL = apiURL;
        Bot = new TelegramBotClient(telegramToken);
        userState = new Dictionary<long, string>();
    }
        ...
        if (update.Type == Telegram.Bot.Types.Enums.UpdateType.Message)
        {
            ...
            if (userState.ContainsKey(message!.Chat.Id) && userState[message.Chat.Id] == "Calculator")
            {
                ...
                if (parameters.Length == 3)
                {
                    ...
                    var url = $"{_apiURL}/Currencies/Calrulator?have={have}&want={want}&amount={amount}";
                    ...
                    {
                        await botClient.SendTextMessageAsync(message.Chat, text: "Enter correct data");
                    }
                }
            ...
            else if (userState.ContainsKey(message!.Chat.Id) && userState[message.Chat.Id] == "Update My Currency")
            {
                ...
                var httpClient = new HttpClient();
                var url = $"{_apiURL}/Currencies/{have}";
            ...
            else
            {
                ...
                else if (message.Text.Length == 3)
                {
                    ...
                    var data = new { have };
                    var httpClient = new HttpClient();
                    var url = $"{_apiURL}/Currencies?have={have}";
                    ...
                    {
                        url = $"{_apiURL}/Currencies/{message.Text}";
                ...
                else if (message.Text == "All My Currency")
                {
                    ...
                    {
                        await botClient.SendTextMessageAsync(message.Chat, text: "😔Sorry, there is no currency in our database.");
                    }
                }
